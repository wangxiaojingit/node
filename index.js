   ...
   methods:{
      ...
      // 微信分享调用接口配置:
       ...
       shareshow:function(item,key,event){
           ...
           //新增逻辑,如果有正在砍价的礼物,就出现弹框,让跳转到我的商品页面
           
//    		if(this.myGoodsNumIing>=1){
...
//    		
           ...
           var gift_id=item.id;
            ...
            axios({
                method:'get',
                url:base+"/xcbb_web/activity/bargain/kanjia-qiang-gou?uid="+uid+"&token="+token+"&round_id="+round_id+"&gift_id="+gift_id+"&union_id="+union_id+"&nick="+nick+"&generation="+generation,
                responseType:'json'
...
//   			    return;
              ...
              var cut1_money= that.cut1_money;
               var leave_money=that.leave_money;
                   //说明在app内
                   window.location.href="./mygoods.html?uid="+uid+"&token="+token+"&kanjia_id="+kanjia_id+"&isShare="+isShare+"&cut1_money="+cut1_money+"&leave_money="+leave_money;
           }
           /*$("#shareText1").html("分享砍掉");
           $("#shareText2").html("快分享给好友帮你砍价吧!~");
           var that=this;
       ...
       goodslitsFn:function(){
           ...
           axios({
               method:'get',
               url:base+"/xcbb_web/activity/bargain//products/list?uid="+uid+"&token="+token+"&union_id="+union_id+"&giftId=0",
               responseType:'json'
             }).then(function(response) {
                 var data=response.data
     ...
     //只请求商品列表,更改刷新数据
       ...
       toMygoods:function(){
           ...
           if(isShare==0){
               //说明在app内
               window.location.href="./mygoods.html?uid="+uid+"&token="+token+"&kanjia_id="+kanjia_id+"&isShare="+isShare;
           }else if(isShare==1){
               //说明在web
